...
import type { StudentProfile, CompanyProfile, Offer, Application, Message, Role, ApplicationStatus } from "./types";
...
function lsSet<T>(k: string, v: T){ if (typeof window!=="undefined") localStorage.setItem(k, JSON.stringify(v)); }
...
export function createOffer(o: Omit<Offer,"id"|"creadaEn">): Offer{
  ...
  all.push(newO); lsSet(K.offers, all); return newO;
}

/* Postulaciones */
export function listApplicationsByStudent(username: string): Application[]{
  const all = lsGet<Application[]>(K.apps, []);
  return all.filter(a => a.estudianteUsername.toLowerCase()===username.toLowerCase()).sort((a,b)=>b.creadaEn-a.creadaEn);
}
export function listApplicationsByCompany(username: string): Application[]{
  const all = lsGet<Application[]>(K.apps, []);
  return all.filter(a => a.empresaUsername.toLowerCase()===username.toLowerCase()).sort((a,b)=>b.creadaEn-a.creadaEn);
}
export function getApplication(appId: string): Application | null {
  const all = lsGet<Application[]>(K.apps, []);
  return all.find(a => a.id===appId) ?? null;
}
export function applyToOffer(params: {
  offer: Offer;
...
export function updateApplicationStatus(appId: string, estado: ApplicationStatus){
  ...
  if (i>=0){ all[i].estado = estado; lsSet(K.apps, all); }
}
...
export function addMessage(appId: string, sender: Role, text: string): Message{
  ...
  list.push(m); lsSet(K.msgs(appId), list); return m;
}
